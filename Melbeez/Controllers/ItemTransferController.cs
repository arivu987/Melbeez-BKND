...
namespace Melbeez.Controllers
{
  ...
  public class ItemTransferController : BaseController
  {
    ...
    public async Task<IActionResult> TransferItem(ItemTransferRequestModel model)
    {
      ...
      {
        return BadRequest("Requested model is not valid.");
      }

      var response = await itemTransferManager.TransferItem(model, User.Claims.GetUserId());

      if (!response.Result)
      {
        return StatusCode(response.StatusCode != 0 ? response.StatusCode : 500, response.Message);
    ...
    public async Task<IActionResult> CancelTransferItem(string transferId, MovedStatus movedStatus)
    {
      ...
      var response = await itemTransferManager.CancelOrRejectTransferItem(transferId, movedStatus, User.Claims.GetUserId());
    ...
    public async Task<IActionResult> ApproveTransferItem(string transferId, bool IsSameLocation, long? locationId)
    {
      ...
      var response = await itemTransferManager.ApproveTransferItem(transferId, User.Claims.GetUserId(), IsSameLocation, locationId);
