...
{
    public class PushNotificationService: IHostedService, IDisposable
    {
        private Timer _timer;
        private readonly IServiceProvider _serviceProvider;
        public PushNotificationService(IServiceProvider serviceProvider)
        {
        ...
        private void Work(object? state)
        {
            try
            {
                WriteLog("Expiry notification service started..");
                using (var scope = _serviceProvider.CreateScope())
                {
                    var _sendNotificationManager = scope.ServiceProvider.GetService<ISendNotificationManager>();
                    _sendNotificationManager.SendWarrentyExpiryNotification("Hosted-System").Wait();
                }
                WriteLog("Expiry notification service ended..");
            }
            catch (Exception ex)
            {
                WriteLog("Error In Expiry notification service " + ex.InnerException == null ? ex.Message : ex.InnerException.Message);
            }
        }
        private TimeSpan getScheduledParsedTime()
        {
            ...
            string jobStartTime = _configuration["AlertSerivceRunTime"];
        ...
        public static void WriteLog(string strLog)
        {
            ...
            logFilePath = logFilePath + "Log-warranty-notification-service" + System.DateTime.Today.ToString("MM-dd-yyyy") + "." + "txt";
